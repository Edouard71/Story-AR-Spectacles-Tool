...
const FrameConstants = {
  ...
  nearFieldInteractionZoneDistance: 32,
...
to manually control frame visibility."
  ...
  set appearance(appearance: FrameAppearance) {
...
moved."
  ...
  set showFollowButton(show: boolean) {
...
precision when interacting with buttons and UI elements using hand tracking."
   ...
   */
  set interactionPlaneOffset(offset: vec3) {
   ...
   */
  set interactionPlanePadding(padding: vec2) {
...
- 2: Ray"
   ...
   */
  get hoverBehavior(): HoverBehavior {
   ...
   */
  get roundedRectangle(): RoundedRectangle {
    return this._roundedRectangle
  }

  private shader: Pass

  collider = this.sceneObject.createComponent("ColliderComponent")
   ...
   */
  readonly onSnappingComplete: PublicApi<void> = this._onSnappingCompleteEvent.publicApi()
   ...
   */
  readonly onScalingUpdate = this._onScalingUpdateEvent.publicApi()
  /**
   ...
   */
  readonly onScalingStart = this._onScalingStartEvent.publicApi()
  ...
  set forceTranslate(forceTranslate: boolean) {
   ...
   */
  set scalingSizeStart(thisSize: vec2) {
   ...
   */
  readonly onInitialized = this._onInitializedEvent.publicApi()
   ...
   */
  readonly onShowVisual = this._onShowVisualEvent.publicApi()
   ...
   */
  readonly onHideVisual = this._onHideVisualEvent.publicApi()
  ...
  initialize() {
    ...
    this._roundedRectangle.gradient = true
    this._roundedRectangle.setBackgroundGradient({
      type: "Linear",
      start: new vec2(-0.66, 1),
      end: new vec2(0.66, -1),
      stop0: {
        enabled: true,
        color: FrameConstants.containerGradientBright,
        percent: 0
      },
      stop1: {
        enabled: true,
        color: FrameConstants.containerGradientDark,
        percent: 0.3
      },
      stop2: {
        enabled: true,
   ...
   */
  setBillboardBufferDegrees(xBufferDegrees: number, yBufferDegrees: number): void {
    if (this.billboardComponent !== null) {
   ...
   */
  set onlyInteractOnBorder(onlyInteractOnBorder: boolean) {
  ...
  private computeHitPosition(interactor: Interactor): HitPosition {
    ...
    if (!this.inputHandler.state.scaling && !this.inputHandler.state.translating && !this._inputState.pinching)
