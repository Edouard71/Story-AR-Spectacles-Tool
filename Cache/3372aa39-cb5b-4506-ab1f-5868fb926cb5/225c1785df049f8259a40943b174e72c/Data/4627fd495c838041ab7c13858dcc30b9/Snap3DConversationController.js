...
let Snap3DConversationController = (() => {
    ...
    var Snap3DConversationController = _classThis = class extends _classSuper {
        ...
        onStart() {
            // 1️⃣ Initialize session controller
            print("🛰️ Initializing SessionController...");
            this.session = SessionController_1.SessionController.getInstance();
            print(`SessionController instance ${this.session}`);
            // 2️⃣ Create SyncEntity and attach properties
            ...
            // 3️⃣ Wait for ready signal
            this.syncEntity.notifyOnReady(() => {
                this.isSyncReady = true;
                print("🔗 SyncEntity ready!");
                // Audience: spawn NPC when host sets prompt
                this.npcPromptProp.onAnyChange.add((newVal) => {
                    print(`[SYNC] npcPromptProp changed → ${newVal}`);
                    if (!this.session.isHost() && newVal && !this.activeNPC) {
                        print(`🎬 Audience spawning synced NPC: ${newVal}`);
                        this.spawnNPC(newVal);
                    }
                });
                // Everyone: play latest spoken line
                this.lastLineProp.onAnyChange.add((line) => {
                    print(`[SYNC] lastLineProp changed → ${line}`);
                    if (line)
                        this.speak(line);
                });
            });
            // 4️⃣ Log your role (host vs audience)
            this.session.onSessionJoined.add(() => {
                const role = this.session.isHost() ? "🎭 Host" : "👀 Audience";
