...
export class Snap3DConversationController extends BaseScriptComponent {
  ...
  this.factory
    ...
    .then((msg: string) => {
      ...
      this.setSystemContext(
        "A futuristic sci-fi lab on Mars, glowing with holographic machinery.",
        "Zyra, a witty alien scientist who studies humans and makes playful observations."
      );
    })
    .catch((err: string) => {
  ...
  private setSystemContext(sceneContext: string, characterDescription: string) {
  const systemPrompt = `
