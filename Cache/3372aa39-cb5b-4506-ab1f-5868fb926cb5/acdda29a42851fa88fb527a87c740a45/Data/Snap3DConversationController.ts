...
export class Snap3DConversationController extends BaseScriptComponent {
  ...
  private npcSyncIdProp = StorageProperty.manualString("npcSyncId", "");
  ...
  private startController() {
    ...
    this.syncEntity = new SyncEntity(
      this,
      new StoragePropertySet([this.npcAssetURLProp, this.npcPromptProp, this.npcSyncIdProp]),
      true,
      "Session",
      null
    );
  ...
  this.npcSyncIdProp.onAnyChange.add((newId: string, oldId: string) => {
    if (newId && newId !== oldId) {
      print(`🔎 Received NPC SyncEntity ID from host: ${newId}`);
      this.waitForHostEntity(newId);
    }
  });
  ...
  private spawnNPC(prompt: string = "Friendly robot with headphones") { 
    ...
    this.factory.createInteractable3DObject(prompt)
      .then(({ msg, assetURL }) => {
        ...
        let npcSyncEntity = SyncEntity.getSyncEntityOnSceneObject(npcObj);
        npcSyncEntity.notifyOnReady(() => {
          const id = npcSyncEntity.networkId;
          print(`🔗 NPC SyncEntity ready (ID: ${id})`);

          // Broadcast this ID so other clients can find the same entity
          if (this.isSyncReady) {
            this.npcSyncIdProp.setPendingValue(id);
            print(`🌍 Broadcasted NPC SyncEntity ID: ${id}`);
          }
        });
  ...
  private waitForHostEntity(syncId: string) {
  let found = false;

  // 1️⃣ Try to find it immediately if it already exists
  const allEntities = this.session.getAllSyncEntities?.() || [];
  for (const e of allEntities) {
    if (e.getNetworkId && e.getNetworkId() === syncId) {
      found = true;
      print(`✅ Found existing host NPC entity (${syncId})`);
      e.notifyOnReady(() => this.attachToHostNPC(e));
      break;
    }
  }

  // 2️⃣ Otherwise wait until it appears
  if (!found) {
    print(`⏳ Waiting for host NPC entity (${syncId})...`);
    this.session.onEntityAdded.add((entity: SyncEntity) => {
      const id = entity.getNetworkId?.();
      if (id === syncId) {
        print(`✅ Host NPC entity appeared (${syncId})`);
        entity.notifyOnReady(() => this.attachToHostNPC(entity));
      }
    });
  }
}

private attachToHostNPC(entity: SyncEntity) {
  print("🔗 Attaching to host NPC SyncEntity...");

  const npcObj = entity.getSceneObject();
  const npcComp = npcObj.getComponent(Snap3DInteractable.getTypeName());
  if (!npcComp) {
    print("❌ Host NPC missing Snap3DInteractable component");
    return;
  }

  this.activeNPC = npcComp;
  this.activeNPC.setSpeechBubble("👋 Connected to host NPC!");
}
