...
export class Snap3DConversationController extends BaseScriptComponent {
  ...
  onStart() {
    ...
    this.session.notifyOnReady(() => {
      print("🛰️ SessionController ready!");
      const role = this.session.isSingleplayer() ? "🧍 Singleplayer" : "🌍 Multiplayer";
      print(`[ConversationController] Session ready in mode: ${role}`);

    // safe to spawn NPC or init SyncEntity here
      this.setupSyncEntity();
    });
  }

  private setupSyncEntity() {
  print("🔗 Setting up SyncEntity...");
