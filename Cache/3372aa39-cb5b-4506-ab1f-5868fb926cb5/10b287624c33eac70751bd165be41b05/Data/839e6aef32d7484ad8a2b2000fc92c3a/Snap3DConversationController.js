...
let Snap3DConversationController = (() => {
    ...
    var Snap3DConversationController = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.npcSyncIdProp = StorageProperty_1.StorageProperty.manualString("npcSyncId", "");
        ...
        startController() {
            ...
            // Set up sync entity
            this.syncEntity = new SyncEntity_1.SyncEntity(this, new StoragePropertySet_1.StoragePropertySet([this.npcAssetURLProp, this.npcPromptProp, this.npcSyncIdProp]), true, "Session", null);
        ...
        setupSyncListeners() {
            ...
            this.npcAssetURLProp.onAnyChange.add((newURL, oldURL) => {
                if (newURL && newURL.length > 0 && newURL !== oldURL) {
                    ...
                    const syncId = this.npcSyncIdProp.currentOrPendingValue;
                    if (syncId) {
                        print(`🔗 Attempting to attach to host NPC with Sync ID: ${syncId}`);
                        const existingEntity = this.session.findEntityByNetworkId(syncId);
                        if (existingEntity) {
                            print("✅ Found host NPC SyncEntity — linking interactable");
                            const npcObj = existingEntity.getSceneObject();
                            if (npcObj) {
                                const comp = npcObj.getComponent(Snap3DInteractable_1.Snap3DInteractable.getTypeName());
                                if (comp) {
                                    this.activeNPC = comp;
                                    comp.setPrompt(prompt);
                                    comp.loadFromURL(newURL);
                                    comp.setSpeechBubble("👋 Synced NPC joined!");
                                    return;
                                }
                            }
                        }
                    }
                    print("⚠️ Could not find host NPC entity — falling back to remote spawn");
        ...
        spawnNPC(prompt = "Friendly robot with headphones") {
            ...
            if (this.isHost || this.role === "Singleplayer") {
                ...
                this.factory.createInteractable3DObject(prompt)
                    ...
                    let npcSyncEntity = SyncEntity_1.SyncEntity.getSyncEntityOnSceneObject(npcObj);
