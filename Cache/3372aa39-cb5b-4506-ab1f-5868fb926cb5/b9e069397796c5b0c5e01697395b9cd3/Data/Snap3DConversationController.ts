...
export class Snap3DConversationController extends BaseScriptComponent {
  ...
  onStart() {
    ...
    this.session.onSessionJoined.add(() => {
      ...
      print(`[Snap3DConversationController] Joined as ${role}`);
    });

    // Wait for sync entity setup
    this.syncEntity.notifyOnReady(() => {
      print("🔗 SyncEntity ready");

      // Listen for synced property changes
      this.syncEntity.onStoragePropertyChanged.add((propName: string, value: any) => {
        if (propName === "npcPrompt" && !this.session.isHost()) {
          print(`🎬 Audience spawning synced NPC: ${value}`);
          this.spawnNPC(value);
        }
      });

      // Listen for network events
      this.syncEntity.onNetworkEventReceived.add((eventName: string, payload: any) => {
        if (eventName === "npcSpoke" && !this.session.isHost()) {
          this.speak(payload);
        }
      });
    });
