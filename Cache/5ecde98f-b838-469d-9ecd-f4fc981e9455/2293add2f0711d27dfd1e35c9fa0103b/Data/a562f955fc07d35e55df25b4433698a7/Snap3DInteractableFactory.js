...
const PropertyType_1 = require("SpectaclesSyncKit.lspkg/Core/PropertyType");
...
let Snap3DInteractableFactory = (() => {
    ...
    var Snap3DInteractableFactory = _classThis = class extends _classSuper {
        ...
        attachSyncToNPC(npcObj) {
            const transform = npcObj.getTransform();
            // Sync position, rotation, and scale
            const transformProp = StorageProperty_1.StorageProperty.forTransform(transform, PropertyType_1.PropertyType.Location, PropertyType_1.PropertyType.Location, PropertyType_1.PropertyType.Local);
            ...
            const colorProp = meshVisual
                ? StorageProperty_1.StorageProperty.forMeshVisualBaseColor(meshVisual, true)
                : null;
            const propSet = new StoragePropertySet_1.StoragePropertySet(colorProp ? [transformProp, colorProp] : [transformProp]);
            const npcScript = npcObj.getComponent(Snap3DInteractable_1.Snap3DInteractable.getTypeName());
            const syncEntity = new SyncEntity_1.SyncEntity(npcScript, propSet, true, "Session", null);
            syncEntity.notifyOnReady(() => {
                print("🪄 NPC SyncEntity ready — mesh shared with all clients");
            });
        }
