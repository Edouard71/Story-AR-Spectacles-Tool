...
export class Snap3DInteractable extends BaseScriptComponent {
  ...
  @input 
  private audio: AudioComponent; // 🔊 new
  @input 
  private textOutput: Text;      // 💬 optional text bubble
  ...
  onAwake() {
    ...
    if (this.promptDisplay) this.promptDisplay.text = "";
    if (this.textOutput) this.textOutput.enabled = true;
  }

  setPrompt(prompt: string) {
    this.promptDisplay.text = prompt;
    this.promptDisplay.getTransform().setLocalPosition(new vec3(0, this.size * 0.6 + 8.0, 0))
  }
