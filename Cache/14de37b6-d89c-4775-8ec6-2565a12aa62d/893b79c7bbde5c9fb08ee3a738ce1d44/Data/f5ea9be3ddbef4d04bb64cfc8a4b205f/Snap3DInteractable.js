        ...
        setupSyncEntity() {
            try {
                ...
                (0, FunctionTimingUtils_1.setTimeout)(() => {
                    const transform = self.getTransform();
                    const transformProp = StorageProperty_1.StorageProperty.forTransform(transform, PropertyType_1.PropertyType.Local, PropertyType_1.PropertyType.Local, PropertyType_1.PropertyType.Local);
                    const meshVisual = self.getSceneObject().getFirstComponent("Component.MaterialMeshVisual");
                    ...
                    self.syncEntity = new SyncEntity_1.SyncEntity(self, propSet, true, "Session", null);
                    self.syncEntity.notifyOnReady(() => {
                        print("🪄 NPC SyncEntity ready — transform + mesh shared with all clients");
                    });
                }, 0); // runs on next frame tick
            }
            catch (e) {
