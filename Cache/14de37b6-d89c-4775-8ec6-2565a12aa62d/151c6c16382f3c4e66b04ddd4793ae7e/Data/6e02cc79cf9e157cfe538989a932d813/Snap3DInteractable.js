...
let Snap3DInteractable = (() => {
    ...
    var Snap3DInteractable = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.remoteMediaModule = require("RemoteMediaModule");
        }
        __initialize() {
            super.__initialize();
            this.modelParent = this.modelParent;
        ...
        setupSyncEntity() {
            ...
            const propSet = new StoragePropertySet_1.StoragePropertySet([transformProp]);
            this.syncEntity = new SyncEntity_1.SyncEntity(this, propSet, true, "Session", null);
            this.syncEntity.notifyOnReady(() => {
                print("🪄 SyncEntity ready on instantiated NPC");
            });
        }
        loadFromURL(url) {
            this.remoteMediaModule.loadResourceAsGltfAsset(url, (gltfAsset) => {
                const settings = GltfSettings.create();
                settings.convertMetersToCentimeters = true;
                gltfAsset.tryInstantiateWithSetting(this.modelParent, this.mat, settings);
